  ...
  <main class="auth-page">
    <section class="auth-panel">
      ...
      <el-form class="auth-form" :model="form" @submit.prevent="submit">
        ...
        <el-form-item>
          <el-input v-model="form.password" placeholder="新密码" type="password" show-password autocomplete="new-password" />
        </el-form-item>
        <el-button class="full-button" type="primary" native-type="submit" :loading="submitting">重置密码</el-button>
      </el-form>
...
async function submit() {
  if (!form.email || !form.code || !form.password) {
    ElMessage.error('请填写邮箱、验证码和新密码')
    return
  }
  submitting.value = true
  try {
    await client.post('/auth/password-reset', { ...form })
